...
export default function Sidebar({
  ...
  const signOutRedirect = async () => {
    // Perform the sign-out action with a delay
    setTimeout(async () => {
      await auth.signOut();
      router.push("/");
    }, 2000);
  ...
  if (loading) {
    return console.log("Carregando suas informações...");
  }
  ...
  let userPhoto = user?.photoURL?.replace("s96-c", "s400-c");
      ...
      >
        <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-neutral-900">
          <div className="flex flex-1 justify-between flex-row items-center">
            <svg
              className="h-12 text-gray-500 dark:text-indigo-600"
              aria-hidden="true"
              xmlns="http://www.w3.org/2000/svg"
              fill="currentColor"
              viewBox="0 0 20 20"
          ...
          <ul className="space-y-2 mt-6 font-medium">
            ...
            <li>
              ...
              >
                ...
                <span
                  className={
                    "inline-flex items-center justify-center w-3 h-3 p-3 ml-3 text-sm font-medium text-white bg-indigo-900 rounded-full dark:bg-indigo-900" +
                    (reqCounter == 0 ? " dark:bg-red-600 dark:text-white" : "")
                  }
                >
                  {reqCounter}
                </span>
            ...
            <li>
              ...
              >
                ...
                <span className="inline-flex items-center justify-center w-3 h-3 p-3 ml-3 text-sm font-medium text-white bg-indigo-900 rounded-full dark:bg-indigo-900">
                  {inboxNotifications}
                </span>
              </a>
            </li>
          ...
          <button onClick={gotoProfile} className="w-full mt-6">
            <a
              href="#"
              className="flex justify-between items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-neutral-800 group"
            >
              <div className="flex items-center">
                <img src={userPhoto} className="w-8 h-8 rounded-full" />
                <span className="flex-1 ml-3 whitespace-nowrap">
                  {displayName}
                </span>
              </div>
              <button
                ...
                className="dark:hover:bg-neutral-900 dark:text-white p-3 transition-all rounded-full hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:focus:ring-gray-600"
              >
                <svg
                  className="flex-shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-white-900 dark:hover:text-white"
                  aria-hidden="true"
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
                  viewBox="0 0 18 16"
